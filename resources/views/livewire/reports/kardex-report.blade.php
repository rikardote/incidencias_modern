...
<div>
    {{-- Loading Overlay --}}
    <div wire:loading wire:target="generate, generateAll, cambiarEmpleado"
        class="fixed inset-0 z-[100] flex items-center justify-center bg-gray-900/20 backdrop-blur-[2px]">
        <div
            class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-2xl flex flex-col items-center gap-4 border border-gray-100 dark:border-gray-700">
            <div class="relative w-16 h-16">
                <div class="absolute inset-0 rounded-full border-4 border-gray-100 dark:border-gray-700"></div>
                <div class="absolute inset-0 rounded-full border-4 border-oro border-t-transparent animate-spin"></div>
            </div>
            <div class="text-center">
                <p class="text-sm font-black text-gray-900 dark:text-gray-100 uppercase tracking-widest">Generando
                    Reporte</p>
                <p class="text-[10px] text-gray-500 dark:text-gray-400 font-bold uppercase mt-1 italic">Este proceso
                    consulta el historial completo...</p>
            </div>
        </div>
    </div>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="flex flex-col md:flex-row items-start md:items-end gap-3 mb-6 w-full">
                        ...
                        <div class="flex gap-2 w-auto shrink-0 mt-3 md:mt-0 items-end">
                            <button wire:click="generate" wire:loading.attr="disabled" @if(!$employee) disabled @endif
                                class="bg-[#13322B] hover:bg-[#0a1f1a] disabled:bg-gray-400 disabled:cursor-wait text-white px-6 py-1.5 rounded text-sm font-bold uppercase tracking-wider transition whitespace-nowrap h-[34px] mt-auto flex items-center gap-2">
                            ...
                            <button wire:click="generateAll" wire:loading.attr="disabled" @if(!$employee) disabled
                                @endif
                                class="bg-oro hover:bg-yellow-600 disabled:bg-gray-400 disabled:cursor-wait text-white px-4 py-1.5 rounded text-sm font-bold uppercase tracking-wider transition whitespace-nowrap h-[34px] mt-auto flex items-center gap-2"
                                ...
                                <span wire:loading wire:target="generateAll" class="flex items-center gap-2">
                                    <svg class="animate-spin h-4 w-4 text-white" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
                                            stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor"
