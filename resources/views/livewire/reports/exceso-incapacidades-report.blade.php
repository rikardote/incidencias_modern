            ...
            $c1 = collect($data)->filter(fn($i) => $i['antiguedad'] < 1)->count();
                $c2 = collect($data)->filter(fn($i) => $i['antiguedad'] >= 1 && $i['antiguedad'] <= 4)->count();
                    $c3 = collect($data)->filter(fn($i) => $i['antiguedad'] >= 5 && $i['antiguedad'] <= 9)->count();
                        ...
                        @elseif(!$loading && $data === null)
                        <div class="flex flex-col items-center justify-center py-24">
                            <div class="w-20 h-20 bg-gray-50 dark:bg-gray-800 rounded-full flex items-center justify-center text-gray-300 dark:text-gray-600 mb-4">
                                <svg class="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                </svg>
                            </div>
                            <h3 class="text-sm font-bold text-gray-600 dark:text-gray-400">Selecciona un departamento</h3>
                            <p class="text-xs text-gray-400 mt-2 text-center">Haz clic en Generar Reporte para comenzar la consulta.</p>
                        </div>
                        @else
                        <!-- Loading State -->
                        <div class="flex flex-col items-center justify-center py-24">
