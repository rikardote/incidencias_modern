<div x-data="{ 
    ...
    "
    class="absolute inset-0 overflow-hidden flex items-center justify-center z-10 w-full h-full clip-path-inset">
    ...
    <div x-show="!jump" 
         x-transition:enter="transition transform ease-out duration-700 delay-[300ms]"
         ...
         <div class="flex items-center justify-center shrink-0">
             <template x-if="islandType === 'success'">
                 <i class="fas fa-check-circle text-[#FFE81F] text-xl drop-shadow-[0_0_8px_rgba(255,232,31,0.6)] animate-pulse"></i>
             </template>
             <template x-if="islandType === 'error'">
                 <i class="fas fa-exclamation-triangle text-red-500 text-xl drop-shadow-[0_0_8px_rgba(239,68,68,0.8)] animate-pulse"></i>
             </template>
         ...
         <div class="overflow-hidden relative flex flex-1 items-center justify-center">
             <span x-text="islandMsg" class="text-[10px] sm:text-[11px] font-black uppercase tracking-widest text-center whitespace-nowrap truncate"
    ...
    <style>
        @keyframes hyperspace-right {
            0% { transform: translateX(5px) scaleX(1); opacity: 0; }
            10% { opacity: 1; }
            100% { transform: translateX(200px) scaleX(5); opacity: 0; }
        }
